...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season_name_key, holiday_name_hash|
    ...
    holiday_name_hash.each do |holiday_name_key, supplies_value|
      capitalize_holiday_name = holiday_name_key.to_s.split("_").each {|holiday|
        holiday.capitalize!}.join(" ")
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.each do |season, holiday|
    holiday.each do |name_of_day, supply_list|
      supply_list.each do |supply_item|
        if supply_item == "BBQ"
          name_of_day
        end
      end
